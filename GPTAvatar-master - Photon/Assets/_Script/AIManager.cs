...
public class AIManager : NetworkBehaviour
{
    ...
    [Rpc(RpcSources.All, RpcTargets.All)]
    void RPC_UpdateChatHistory(string role, string content)
    {
    ...
    void OnGPT3TextCompletedCallback(RTDB db, JSONObject jsonNode)
    {
        ...
        if (reply.Length < 5)
        {
            ...
            RPC_SayText(db.GetString("english"));
            return;
        }
        ...
        RPC_UpdateChatHistory(db.GetString("role"), db.GetString("question"));
        RPC_UpdateChatHistory("assistant", reply);
    }

    [Rpc(RpcSources.All, RpcTargets.All)]
    void RPC_SayText(string text)
    {
